...
function VideoEditorWithStore({
  ...
  insertEditRef.current = () => sourceMonitorActionsRef.current?.dispatchKeyboardAction('edit.insertEdit')
  overwriteEditRef.current = () => sourceMonitorActionsRef.current?.dispatchKeyboardAction('edit.overwriteEdit')
  ...
  const extractCurrentFrame = useCallback(async (clip: TimelineClip): Promise<string | null> => {
    try {
      ...
      if (clip.type === 'video') {
        ...
        const result = await window.electronAPI.extractVideoFrame(clipPath, seekTime, 1024, 2)
        return result.path
      }
      return clipPath
    } catch (err) {
  ...
  const handleRetakeClip = useCallback((clip: TimelineClip) => {
    ...
    setGenSpaceRetakeSource({
      videoUrl: getAssetMediaUrl(liveAsset),
      videoPath: takePath,
      clipId: clip.id,
