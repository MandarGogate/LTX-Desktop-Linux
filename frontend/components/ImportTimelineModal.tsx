...
function getRefPath(ref: ParsedMediaRef): string {
  return ref.path?.trim() || ref.resolvedPath.trim()
}
  ...
  const handleSearchDirectory = useCallback(async () => {
    ...
    try {
      ...
      for (const ref of missingRefs) {
        const filename = ref.name || getFilenameFromPath(getRefPath(ref))
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    try {
      for (const ref of refsToImport) {
        const sourcePath = getRefPath(ref)
