...
class TestConditioningCache:
    def test_get_returns_none_for_missing_key(self):
        ...
        key = ConditioningCacheKey("video.mp4", "canny", 768, 432, 97, 97)
    ...
    def test_cleanup_removes_files_and_clears(self, tmp_path: Path):
        ...
        cache.put(
            ConditioningCacheKey("a.mp4", "canny", 768, 432, 97, 97),
            ConditioningCacheEntry(str(file1), 10, 24.0),
        )
        cache.put(
            ConditioningCacheKey("b.mp4", "depth", 768, 432, 97, 97),
        ...
        assert cache.get(ConditioningCacheKey("a.mp4", "canny", 768, 432, 97, 97)) is None

    def test_cleanup_tolerates_already_deleted_files(self, tmp_path: Path):
        cache = ConditioningCache()
        missing = tmp_path / "gone.mp4"
        cache.put(
            ConditioningCacheKey("v.mp4", "canny", 768, 432, 97, 97),
